...
#
# Source the environment (partially needed)
...
TMP=`hostname -s`
NODE=`echo ${TMP:(-1)}`
NETMASK=`ifconfig ens3 | egrep netmask | sed s/'\(.*\)netmask \([0-9]*\.[0-9]*\.[0-9]*\.[0-9]*\)\(.*\)'/"\2"/g`
...
then
	...
	/sbin/ifconfig ens3:1 ${IP} netmask ${NETMASK}
	checkExitCode $?
else
...
then
	...
	/sbin/ifconfig ens3:2 ${IP} netmask ${NETMASK}
	checkExitCode $?
else
...
then
	...
	/sbin/ifconfig ens3:3 ${IP} netmask ${NETMASK}
	checkExitCode $?
else
